...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--headers', action='store_true', help="whether the file was created in verbose mode, storing record headers")
            ...
            if(not conf.isStreamContinuous):
                ...
                if(parser.verify):
                    for i in range(record_length):
                        if(samples[i] != (i+1)*(conf.channel+1)):
                            print("Mismatch detected: record(#{}) sample {} should be {}, is {}.".format(head.recordNumber, i, (i+1)*(conf.channel+1), samples[i]))
