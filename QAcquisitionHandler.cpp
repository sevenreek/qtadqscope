...
bool QAcquisitionHandlerGen3::requestStart(
  ...
  this->bufferProcessor =
      std::unique_ptr<BufferProcessorGen3>(new BufferProcessorGen3(
          *this->adq, *this->config, *this->recordProcessors,
  ...
  this->connect(this, &QAcquisitionHandlerGen3::requestProxyStart,
                this->processorProxy.get(), &QBufferProcessorProxy::startLoop,
                Qt::QueuedConnection);
  this->connect(this, &QAcquisitionHandlerGen3::requestProxyStop,
                this->processorProxy.get(), &QBufferProcessorProxy::stop,
                Qt::QueuedConnection);
  this->connect(this->processorProxy.get(),
                &QBufferProcessorProxy::stateChanged, this,
                &QAcquisitionHandlerGen3::onProcessorStateChanged, Qt::QueuedConnection);
  this->startDelayedAcquisition();
  return true;
}
bool QAcquisitionHandlerGen3::requestStop() { emit this->requestProxyStop(); return true;}
...
void QAcquisitionHandlerGen3::startDelayedAcquisition()
{
...
bool QAcquisitionHandlerGen3::configure(AcquisitionConfiguration *acq) {
  ...
  for (int ch = 0; ch < MAX_NOF_CHANNELS; ch++) {
    if (acq->collection.channelMask() & (1 << ch)) {
      ...
      if (!this->adq->SetInputRange(
              adqch, acq->AFEs.at(ch).desiredInputRange(), &res)) {
      ...
      if (!this->adq->SetGainAndOffset(
              adqch, acq->calibrations.at(ch).digitalGain(),
    ...
    if (!this->adq->SetChannelSampleSkip(ch + 1,
                                         acq->collection.sampleSkip())) {
  ...
  if (this->adq->InitializeParameters(
          ADQParameterId::ADQ_PARAMETER_ID_DATA_ACQUISITION, &acqParams) !=
      sizeof(acqParams)) // this is an error check
  {
  ...
  for (int ch = 0; ch < MAX_NOF_CHANNELS; ch++) {
    ...
    acqParams.channel[ch].horizontal_offset =
        acq->triggers.at(ch).horizontalShift();
    acqParams.channel[ch].trigger_edge =
        static_cast<ADQEdge>(acq->triggers.at(ch).edge());
    ...
    this->trigSetupHelperConfig.levelArray[ch] =
        static_cast<int>(acq->triggers.at(ch).level());
    this->trigSetupHelperConfig.resetArray[ch] =
        static_cast<int>(acq->triggers.at(ch).reset());
    this->trigSetupHelperConfig.edgeArray[ch] =
        static_cast<int>(acq->triggers.at(ch).edge());
  }
  ...
  if (this->adq->InitializeParameters(
          ADQParameterId::ADQ_PARAMETER_ID_DATA_TRANSFER, &dtParams) !=
      sizeof(dtParams)) {
  ...
  if (this->adq->InitializeParameters(
          ADQParameterId::ADQ_PARAMETER_ID_DATA_READOUT, &roParams) !=
      sizeof(roParams)) {
  ...
  if (!this->adq->SetupLevelTrigger(
          trigSetupHelperConfig.levelArray, trigSetupHelperConfig.edgeArray,
          trigSetupHelperConfig.resetArray,
          0, // 0,// the example uses 0 for some reason, might want to use
             // config->collection->channelMask
  ...
  for (int ch = 0; ch < MAX_NOF_CHANNELS; ch++) {
    if (!this->adq->SetChannelTriggerMode(
            ch + 1, static_cast<int>(acq->triggers.at(ch).mode()))) {
...
void QAcquisitionHandlerGen3::onFullyStopped()
{
