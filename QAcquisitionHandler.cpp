...
QAcquisitionHandlerGen3::QAcquisitionHandlerGen3(ADQInterface *adq, QObject *parent)
  ...
  connect(&this->acquisitionTimer, &QTimer::timeout, this,
          &QAcquisitionHandlerGen3::stopAcquisition,
  ...
  this->acquisitionStabilizationTimer.connect(
      ...
      &QAcquisitionHandlerGen3::startAcquisition,
      Qt::ConnectionType::QueuedConnection);
}
...
void QAcquisitionHandlerGen3::finishRecordProcessors() {
  for (auto rp : *this->recordProcessors) {
    ...
    double dataRate = (processedBytes / 1.0e6) /
                      (this->config->collection.duration() / 1.0e3);
    if (processedBytes)
      spdlog::info("{} processed {:.4f} GBs. Data rate {:.4f} MB/s.",
                   rp->getName(), processedBytes / 1.0e9, dataRate);
  }
}
bool QAcquisitionHandlerGen3::configure(AcquisitionConfiguration *acq) {
  ...
  for (int ch = 0; ch < MAX_NOF_CHANNELS; ch++) {
    if (acq->collection.channelMask() & (1 << ch)) {
      int adqch = ch + 1;
      if (!this->adq->SetInputRange(adqch, acq->AFEs.at(ch).desiredInputRange(),
                                    &res)) {
  ...
  if (this->adq->InitializeParameters(
          ADQParameterId::ADQ_PARAMETER_ID_DATA_ACQUISITION,
  ...
  this->acquisitionStabilizationTimer.setInterval(
      acq->collection.stabilizationDelay());
