...
QConfigurationController::QConfigurationController(QObject* parent) : QObject(parent)
{
...
AcquisitionConfiguration QConfigurationController::acqFromV0JSON(QJsonObject& json)
{
    // handle legacy code...
    throw std::runtime_error("Legacy JSON loading not implemented yet.");
}
bool QConfigurationController::loadAcquisitionFromFile(const std::string& path)
{
...
{
    this->acq().collection.setListenerCallback([this]() { this->notifyCollectionChanged(); });
    this->acq().spectroscope.setListenerCallback([this]() { this->notifySpectroscopeChanged(); });
    this->acq().storage.setListenerCallback([this]() { this->notifyStorageChanged(); });
    this->acq().transfer.setListenerCallback([this]() { this->notifyTransferChanged(); });
    for (int ch = 0; ch < MAX_NOF_CHANNELS; ch++)
    {
        this->acq().AFEs.at(ch).setListenerCallback([this, ch]() { this->notifyAFEChanged(ch); });
